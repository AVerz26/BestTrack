...
if botao and origem and destino:
    with st.spinner("Calculando rota..."):
        try:
            ...
            st.session_state.duracao = round(st.session_state.dist_km / valor, 1)
...
if st.session_state.rota_mapa:
    ...
    if st.session_state.dist_km:
        st.success(f"🛣️ Distância total: {st.session_state.dist_km} km - Tempo estimado: {st.session_state.duracao} horas. ")
